...
import { ChangeEvent, ComponentType, KeyboardEvent, useCallback, useEffect, useLayoutEffect, useRef, useState } from 'react';
...
export default function EditPost({ data, categories }: Props) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (user && data.id) {
      ...
      const res = await PostsService.update(values, data.id)
      if (!res.error) {
  ...
  useLayoutEffect(() => {
    ...
    const convertedFromRay = convertFromRaw(parsedContent)
    // const contentState = ContentState.createFromBlockArray(
    //   htmlToDraft(parsedContent) as any,
    // );
    const editState = EditorState.createWithContent(convertedFromRay);
    setEditorState(editState);
  }, [data]);

  useEffect(() => {
    //@ts-ignore
    form.setValue('slug', slug.replaceAll(' ', '-').replace(/![^\p{L}\s._0-9\-]+/g, '').toLowerCase().normalize('NFD').replaceAll(/[\u0300-\u036f]/g, ""))
  }, [slug]);
                ...
                <FormItem className="w-full flex justify-between items-center">
                  ...
                  <FormControl>
                    <Switch
                      size="sm"
                      checked={field.value}
                      defaultSelected={data.published}
